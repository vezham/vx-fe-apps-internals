...
// Add the showPromoBanner prop interface
interface NavbarDemoProps {
  showPromoBanner?: boolean
}

const Navbars = ({ showPromoBanner = false }: NavbarDemoProps) => {
  // Define navigation items in the format expected by DropdownNavigation
  const NAV_ITEMS = [
    {
      id: 1,
      label: 'Products',
  ...
  // Update the top position based on promo banner visibility
            ...
            <div className="max-h-[60vh] w-full overflow-y-auto">
              {NAV_ITEMS.map((navItem, index) =>
                navItem?.subMenus ? (
                  ...
                  <div
                    key={index}
                    className="mb-2 flex w-full cursor-pointer flex-col gap-2 px-2">
